...
# Plot Domainograms (version 0.1.6)
...
# 0.0.8 - Scale the data using Z-score
...
rcmd_running <- function() {
  nchar(Sys.getenv('R_TESTS')) != 0
        ...
        smooth.DamID<-function(DamID.GR, ws=201, pseud=10, title=NULL)
        ...
        plot_domainograms_with_ggplot = function(dg_exp, del, annot, my_genes, 
                ...
                for(this_col in 1:length(colnames(focused_df))){ 
                        ...
                        if(nrow(tmp[tmp$score>=0.95,]) == 0){
                        #if(nrow(tmp %>% filter(score >= 0.95)) == 0){
                                ...
                                #ecdf_exp = rbind(ecdf_exp, tmp %>% filter(score<=0.05))
                        #} else if(!any(tmp$ECD<=0.05)){
                        } else if(nrow(tmp[tmp$score<=0.05,]) == 0){
                        #}  else if(nrow(tmp %>% filter(score<=0.05)) == 0) {
                                ...
                                # ecdf_ctl = rbind(ecdf_ctl, tmp %>% filter(score>=0.95))
                ...
                domainogram_plot = 
                        ...
                        scale_y_discrete(
                                ...
                                # limits = levels(factor(ecdf_exp$Window))
                                limits = y_axis,
                                # get this labels...
                                breaks = y_axis[b != ""],
...
#                     '---------------------""""---------------------'
        ...
        save(sDamID.Exp, sDamID.Control, SMTH, data.summ,
             DG.Exp, mygenes, Refseq.mm10, Del, file = CHECKPOINT)

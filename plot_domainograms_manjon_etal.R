...
# Plot Domainograms (version 0.1.0)
## Vinicius Franceschini-Santos, Lise Dauban 230411
...
# 0.1.0 - Color the upper-domainogram by the delta, not by the quantile value
...
rcmd_running <- function() {
  nchar(Sys.getenv('R_TESTS')) != 0
        ...
        logger(paste0("Rscript plot_domainograms.R",
                      ...
                      "\n\t--recombination_end ", recombination_end,
        ...
        emp.domainogram <- function(damid.exp, damid.ctl, chrom, window.sizes=c(101), 
                                    ...
                                    loess.corr=NULL, baseline=0, do_ecdf=T, output_dir=NULL)
                ...
                # metadata (accessible via mcols() function)
        {
                ...
                for(w.size in window.sizes){
                        ...
                        # vf20230217: added do_ecdf variable. will get the ECD 
                        #             only if do_ecdf is TRUE -- otherwise return
                        #             the raw values
                        if(do_ecdf==T){
        ...
        plot_domainograms_with_ggplot = function(dg_exp, dg_exp_raw, del, annot, my_genes, 
                ...
                if(recombination == "deletion"){
                        ...
                        damid_df_wide_b =
                                damid_df_wide %>%
                                filter(chr_position <= recombination_start)
                        
                        damid_df_wide_a =
                                damid_df_wide %>%
                                filter(chr_position >= recombination_end)
                ...
                if(recombination == "deletion"){
                        damid_plot =
                                ...
                                geom_segment(aes(x = x, 
                                             ...
                                             show.legend = F) +
                                # Add dashed lines
                                geom_vline(xintercept = recombination_start,
                                           linetype = "dashed",
                                           color = "#00a087") +
                                geom_vline(xintercept = recombination_end,
                                           linetype = "dashed",
                                           color = "#00a087") +
                                ...
                                geom_hline(yintercept = Q095,
                                           ...
                                           color = "gray80") +
                                ggthemes::theme_few() +
                                ylab(expression(log[2]~(DamID~ratio))) +
                ...
                dgsub = dg_exp[(start(dg_exp)>=x_range[1] 
                                & end(dg_exp)<=x_range[2])]
                
                # transforming the output of 'calculate domainograms' in dataframe
                genome_wide_df = as.data.frame(mcols(dg_exp))
                focused_df = as.data.frame(mcols(dgsub))
                ################
                ################
                #print("FOCUSED DF -- ECDF")
                #print(head(focused_df))
                
                ### Same but for raw differences (not ecdf)
                dgsub_raw = dg_exp_raw[(start(dg_exp_raw)>=x_range[1] 
                                & end(dg_exp_raw)<=x_range[2])]
                
                # transforming the output of 'calculate domainograms' in dataframe
                #genome_wide_df_raw = as.data.frame(mcols(dg_exp_raw))
                focused_df_raw = as.data.frame(mcols(dgsub_raw))
                
                ################
                ################
                #print("FOCUSED DF -- RAW DIFFERENCES")
                #print(head(focused_df_raw))
                ...
                for(this_col in 1:length(colnames(focused_df))){ 
                        ...
                        # get raw difference
                        difference = mcols(dgsub_raw)[,this_col][mcols(dgsub_raw)[,this_col]!=2]
                ...
                if(recombination == "inversion" | recombination == "deletion"){
                        recomb = data.frame()
                        for(this_col in 1:length(colnames(focused_df))){ 
                ...
                domainogram_plot = 
                        ...
                        geom_tile(aes(x = start, 
                                      ...
                                      color = score), 
                                  data = ecdf_exp) +
                        scale_color_distiller(palette = "Reds", name=" ", direction=-1, 
                                              limits = c(0,0.05)) +
                        new_scale_color()+
                        geom_tile(aes(x = start, 
                                      y = factor(Window), 
                                      color = score), 
                                  data = ecdf_ctl) +
                        scale_color_distiller(palette = "Blues", direction = 1, name=" ",
                                              limits = c(0.95, 1)) +
                        ggthemes::theme_few()+
                        scale_x_continuous(limits = x_range,
                                           expand = c(0, 0)) +
                        scale_y_discrete(
                                ...
                                limits = levels(factor(ecdf_ctl$Window)),
                                # get this labels...
                                breaks = levels(factor(ecdf_ctl$Window))[b != ""],
                ...
                if(recombination == "inversion"){
                        # rotate/flip/invert genes within the inversion
                        
                        distance_from_start_st = (rna_seq_data[(rna_seq_data$start > recombination_start 
                                                                & rna_seq_data$stop < recombination_end),]$start
                                                  - recombination_start)
                        
                        distance_from_start_en = (rna_seq_data[(rna_seq_data$start > recombination_start 
                                                                & rna_seq_data$stop < recombination_end),]$stop
                                                  - recombination_start)
                        
                        
                        new_ends = (distance_from_start_en - recombination_end )*-1
                        new_starts = (distance_from_start_st - recombination_end )*-1
                        
                        rna_seq_data[(rna_seq_data$start > recombination_start
                                      & rna_seq_data$stop < recombination_end),"start"] = new_starts
                        
                        rna_seq_data[(rna_seq_data$start > recombination_start
                                      & rna_seq_data$stop < recombination_end),"stop"] = new_ends
                ...
                ########################################################################
                ## Combined plot -- final
                ## VF230411 - combining the legends
                #leg_dom = get_legend(domainogram_plot)
                #save(domainogram_plot, damid_plot, RNASeq_plot, file = "VF230411_TXR3_vs_DPURO3/plots.RData")
                
                leg1 <- get_legend(domainogram_plot+theme(legend.box = "horizontal"))
                leg2 <- get_legend(damid_plot)
                #leg3 <- get_legend(RNASeq_plot)
                
                # create a blank plot for legend alignment 
                blank_p <- plot_spacer() + theme_void()
                
                # combine legend 2 & blank plot
                leg30 <- plot_grid(leg2, blank_p, 
                                   blank_p, 
                                   nrow = 3
                )
                
                leg12 <-plot_grid(leg1)
                leg12 = plot_grid(leg1)
                
                plt = domainogram_plot / damid_plot / RNASeq_plot + 
                ...
                leg123 <- plot_grid(leg12, leg30,
                                    nrow = 2,
                                    labels = "Genome-wide quantile", 
                                    label_size = 12, vjust=7, hjust = -0.25,
                                    label_fontface = "bold"
                )
                
                final_plot = plot_grid(plt, leg123,
                          nrow = 1,
                          align = "h",
                          axis = "t",
                          rel_widths = c(1, 0.3))
...
#                     '---------------------""""---------------------'
        ...
        Del<-data.frame(seqnames=recombination_chr,
                        start=c(recombination_start, recombination_end),
                        end=c(recombination_start+1, recombination_end+1))
        ...
        } else if(recombination == "inversion"){
                logger("-----------------------------------------------------------")
                logger("Parsing the inversion (--recombination inversion)")
                # Invert sequences for Exp only
                DamID.Exp[seqnames(DamID.Exp) == recombination_chr & start(DamID.Exp) > Del$start[1] & end(DamID.Exp)< Del$end[2]]$Dam <- rev(DamID.Exp[seqnames(DamID.Exp) == recombination_chr & start(DamID.Exp) > Del$start[1] & end(DamID.Exp)< Del$end[2]]$Dam)
                DamID.Exp[seqnames(DamID.Exp) == recombination_chr & start(DamID.Exp) > Del$start[1] & end(DamID.Exp)< Del$end[2]]$DamLam <- rev(DamID.Exp[seqnames(DamID.Exp) == recombination_chr & start(DamID.Exp) > Del$start[1] & end(DamID.Exp)< Del$end[2]]$DamLam)
        ...
        logger("Smoothing")
        #smooth:
        sDamID.Control<-smooth.DamID(DamID.Control, ws=SMTH, pseud=30) #slightly higher pseudocount because of data sparsity
        sDamID.Exp<-smooth.DamID(DamID.Exp, ws=SMTH, pseud=30)
        # Combine the smoothed with the deleted GRanges
        if(recombination == "deletion") {
                logger("Combine the smoothed with the deleted data (--recombination deletion)")
        ...
        if(recombination == "deletion"){
                ...
                DamID.Control[(seqnames(DamID.Control) == recombination_chr 
                               & start(DamID.Control) > Del$start[1] 
                               & end(DamID.Control)< Del$end[2])]$Dam <- 
                        rev(DamID.Control[(seqnames(DamID.Control) == recombination_chr 
                                           & start(DamID.Control) > Del$start[1] 
                                           & end(DamID.Control)< Del$end[2])]$Dam)
                DamID.Control[seqnames(DamID.Control) == recombination_chr & start(DamID.Control) > Del$start[1] & end(DamID.Control)< Del$end[2]]$DamLam <- rev(DamID.Control[seqnames(DamID.Control) == recombination_chr & start(DamID.Control) > Del$start[1] & end(DamID.Control)< Del$end[2]]$DamLam)
        ...
        # Same as above, but now, get raw difference, not the ECDF
        DG.Exp_raw<-emp.domainogram(damid.exp=DamID.Exp, 
        ...
        if(recombination != "FALSE") {
                ...
                for(i in colnames(mcols(DG.Exp))){
                        ...
                        mcols(DG.Exp_raw[(queryHits(ovl.dom)[1]-(g+1)/2):(queryHits(ovl.dom)[1]+(g+1)/2)])[i] <- 2
                        mcols(DG.Exp_raw[(queryHits(ovl.dom)[2]-(g+1)/2):(queryHits(ovl.dom)[2]+(g+1)/2)])[i] <- 2
        ...
        if(recombination == "deletion"){
                ...
                mcols(DG.Exp_raw[(queryHits(ovl.dom)[1]):(queryHits(ovl.dom)[2])]) <- 2
        ...
        if(recombination == "inversion"){
                ...
                sDamID.Control[seqnames(sDamID.Control) == recombination_chr & start(sDamID.Control) > Del$start[1] & end(sDamID.Control)< Del$end[2]]$logratio <- rev(sDamID.Control[seqnames(sDamID.Control) == recombination_chr & start(sDamID.Control) > Del$start[1] & end(sDamID.Control)< Del$end[2]]$logratio)
                
                
                # remove values at the junction, only for Ctrl. To show that is 
                # the WT the lines are not connected.
                ovl = findOverlaps(sDamID.Control, Del.gr)
        ...
        save(sDamID.Exp, sDamID.Control, SMTH, data.summ,
             DG.Exp, DG.Exp_raw, mygenes, Refseq.mm10, Del, file = CHECKPOINT)
...
option_list <- list(
        ...
        make_option(c("--my_genes"), type="character", default="Tdgf1", 
                    ...
                    metavar="gene_name"),
        make_option(c("--recombination_start"), type="integer", default=110927601, 
                    help="Start coordinate of the recombined region (default: 110927601)", 
                    metavar="coordenate"),
        make_option(c("--recombination_end"), type="integer", default=112440815, 
...
main(smooth_window_size = opt$smooth_window_size,
     ...
     recombination_start = opt$recombination_start,
     recombination_end = opt$recombination_end,
     ...
     recombination = opt$recombination,
